...
sub parse_file {
    ...
    printf("interval read-iops write-iops read-mbps write-mbps avg-read-iops avg-write-iops\n");
    ...
    while (<inFILE>) {
        ...
        if ($line =~ $deviceName) {
            ...
            my $this_read_mb_per_sec = $line_values[5];
            my $this_write_mb_per_sec = $line_values[6];

            $num_measurements++;
            $total_read_per_sec += $this_read_per_sec;
            $total_write_per_sec += $this_write_per_sec;
            
            printf("%d %.2f %.2f %.2f %.2f %.2f %.2f\n", $num_measurements*$iostatInterval, $this_read_per_sec, $this_write_per_sec, $this_read_mb_per_sec, $this_write_mb_per_sec, $total_read_per_sec/$num_measurements, $total_write_per_sec/$num_measurements);
        }
    }
...
if (@ARGV == 0) {
    print "usage: parse_iostat.pl <input-file-name> <device-name> <iostat-interval>\n";
    exit;
}

$file_name = $ARGV[0];
$device_name = $ARGV[1];
$iostat_interval = $ARGV[2];

parse_file($file_name,$device_name,$iostat_interval);
