...
for benchType in PP PS RP RS; do
    if [ ${benchType} == "PP" ]; then
        echo "POINT.PRIMARY"
    elif [ ${benchType} == "PS" ]; then
        echo "POINT.SECONDARY"
    elif [ ${benchType} == "RP" ]; then
        echo "RANGE.PRIMARY"
    elif [ ${benchType} == "RS" ]; then
        echo "RANGE.SECONDARY"
    fi

    for i in ${fullPath}/*${benchName}*${benchType}*.log; do
        baseName=$(basename $i)
