            ...
            if ($line ne "") {
                ...
                if ($line_values[0]) {
                    if ($line_values[0] eq "") {
                        ...
                        my $this_thruput = $line_values[1];
                        my $this_reads = $line_values[2];
                        my $this_writes = $line_values[3];
                        ...
                        if ($parse_type eq "tps") {
                            print outFILE "$this_seconds $this_thruput $this_reads $this_writes $this_latency\n";
                        }
                    }
                } else {
                    $bad_lines++;
...
if (@ARGV == 0) {
    print "usage: parse_sysbench.pl <sql/tps/summary> <benchmark date in yyyy-mm-dd> <starting pk> <output file name> <input file spec>\n";
    exit;
}
...
if (($parse_type eq "sql") && ($num_args != 5)) {
    print "usage: parse_sysbench.pl sql <benchmark date in yyyy-mm-dd> <starting pk> <output file name> <input file dir>\n";
    exit;
} elsif (($parse_type eq "tps") && ($num_args != 2)) {
    print "usage: parse_sysbench.pl tps <input file dir>\n";
    exit;
} elsif (($parse_type eq "summary") && ($num_args != 2)) {
    print "usage: parse_sysbench.pl summary <input file dir>\n";
    exit;
} elsif (($parse_type ne "sql") && ($parse_type ne "tps") && ($parse_type ne "summary")) {
    print "usage: parse_sysbench.pl sql/tps/summary\n";
    exit;
}    
...
foreach my $file (@files) {
    ...
    if ("debug" eq "true") {
        print "\n";
        print "  filename         = $filename_without_path_or_extension\n";
        print "  server_name      = $parm_server_name\n";
        print "  database_name    = $parm_database_name\n";
        print "  database_version = $parm_database_version\n";
        print "  engine_name      = $parm_engine_name\n";
        print "  engine_version   = $parm_engine_version\n";
        print "  benchmark_name   = $parm_benchmark_name\n";
        print "  rowcount         = $parm_rowcount\n";
        print "  threads          = $parm_threads\n";
        print "  commit_sync      = $parm_commit_sync\n";
        print "  extra_info       = $parm_extra_info\n";
        print "\n";
    }
    ...
    parse_file($file,$parm_server_name,$parm_benchmark_name,$parm_benchmark_date,$parm_engine_name,$parm_engine_version,$parm_database_name,$parm_database_version,$parm_commit_sync,$parm_starting_pk,$parm_extra_info,$parm_rowcount,$parm_threads,$filename_without_path_or_extension,$parse_type);
