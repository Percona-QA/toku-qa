...
if [ -z "$MONGO_USERNAME" ]; then
    echo "Need to set MONGO_USERNAME"
    exit 1
fi
if [ -z "$MONGO_PASSWORD" ]; then
    echo "Need to set MONGO_PASSWORD"
    exit 1
fi
...
echo "`date` | Adding users" | tee -a $LOG_NAME
mongo-create-users-24
mongo-create-users-26

DB_NUM=1
while [ ${DB_NUM} -le ${NUM_LOADER_THREADS} ]; do
    thisDb=${DB_NAME}${DB_NUM}
    echo "`date` | creating user in ${thisDb}" | tee -a $LOG_NAME
    $MONGO_DIR/bin/mongo --username=admin --password=admin admin --eval "db=db.getSiblingDB(\"${thisDb}\"); printjson(db.addUser( { user: \"${MONGO_USERNAME}\", pwd: \"${MONGO_PASSWORD}\", roles: [ \"readWrite\" ] } ))"
    $MONGO_DIR/bin/mongo --username=admin --password=admin admin --eval "db=db.getSiblingDB(\"${thisDb}\"); printjson(db.createUser( { user: \"${MONGO_USERNAME}\", pwd: \"${MONGO_PASSWORD}\", roles: [ \"readWrite\" ] } ))"
    let DB_NUM=DB_NUM+1
done
