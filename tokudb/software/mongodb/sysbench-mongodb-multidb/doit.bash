...
if [ -z "$MONGO_USERNAME" ]; then
    export MONGO_USERNAME=tmc
fi
if [ -z "$MONGO_PASSWORD" ]; then
    export MONGO_PASSWORD=tmc
fi
if [ -z "$SEED" ]; then
    export SEED=$( date +%s )
fi
...
tarFileName="${MACHINE_NAME}-${BENCHMARK_NUMBER}-${DATE}-mongoSysbench-${MONGO_TYPE}-${NUM_COLLECTIONS}-${NUM_DOCUMENTS_PER_COLLECTION}-${BENCH_ID}${BENCHMARK_SUFFIX}.tar.gz"
