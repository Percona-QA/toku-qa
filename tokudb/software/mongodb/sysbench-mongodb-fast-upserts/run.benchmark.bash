...
for threadCount in ${threadCountList}; do
    ...
    if [ ${CAPTURE_PMPROF} == "Y" ]; then
        PMPROF_TOTAL_SECONDS=200
        PMPROF_SECONDS_BETWEEN_SAMPLES=1
        PMPROF_NUMBER_OF_RUNS=1
        PMPROF_PROCESS_NAME=mongod
        PMPROF_SECONDS_DELAY=20

        printf "`date` | starting pmprof in $PMPROF_SECONDS_DELAY second(s)"
...
echo "`date` | Grand Total | ${sumTotal}" | tee -a $LOG_NAME
