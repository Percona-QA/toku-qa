  ...
  /* Parse args */
    ...
    while ( (c = getopt(argc, argv, "h:P:S:d:u:p:w:c:r:l:i:f:t:n:")) != -1) {
        ...
        case 'P':
            ...
            break;
        case '?':
    	    printf("Usage: tpcc_start -h server_host -P port -S socket -d database_name -u mysql_user -p mysql_password -w warehouses -c connections -r warmup_time -l running_time -i report_interval -f report_file -t trx_file\n");
            exit(0);
        default:
