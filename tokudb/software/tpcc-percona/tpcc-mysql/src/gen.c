...
static void 
PrintOneBind(MYSQL_BIND *param, FILE *out_file) {
        switch (param->buffer_type) {
        case MYSQL_TYPE_LONG:
                fprintf(out_file, "%d", *(int *)param->buffer); // note: MYSQL_TYPE_LONG is an int
                break;
        case MYSQL_TYPE_STRING:
                fprintf(out_file, "%s", param->buffer);
                break;
        case MYSQL_TYPE_FLOAT:
                fprintf(out_file, "%f", *(float *)param->buffer);
                break;
        default:
                assert(0);
        }
...
static void 
PrintBind(MYSQL_BIND *param, int n, FILE *out_file) {
        int i;
        for (i = 0; i < n; i++) {
                PrintOneBind(&param[i], out_file);
                if (i < n-1)
                    fprintf(out_file, "\t");
        }
        fprintf(out_file, "\n");
...
static FILE *warehouse_file;
static FILE *stock_file;
static FILE *district_file;
static FILE *customer_file;
static FILE *history_file;
static FILE *orders_file;
static FILE *new_orders_file;
static FILE *order_line_file;

static void OpenFiles(void) {
        item_file = fopen("item.txt", "w"); assert(item_file);
        warehouse_file = fopen("warehouse.txt", "w"); assert(warehouse_file);
        stock_file = fopen("stock.txt", "w"); assert(stock_file);
        district_file = fopen("district.txt", "w"); assert(district_file);
        customer_file = fopen("customer.txt", "w"); assert(customer_file);
        history_file = fopen("history.txt", "w"); assert(history_file);
        orders_file = fopen("orders.txt", "w"); assert(orders_file);
        new_orders_file = fopen("new_orders.txt", "w"); assert(new_orders_file);
        order_line_file = fopen("order_line.txt", "w"); assert(order_line_file);
}

static void CloseFiles(void) {
        fclose(item_file); item_file = NULL;
        fclose(warehouse_file); warehouse_file = NULL;
        fclose(stock_file); stock_file = NULL;
        fclose(district_file); district_file = NULL;
        fclose(customer_file); customer_file = NULL;
        fclose(history_file); history_file = NULL;
        fclose(orders_file); orders_file = NULL;
        fclose(new_orders_file); new_orders_file = NULL;
        fclose(order_line_file); order_line_file = NULL;
}
	...
	char           *argv[];
{
	...
	"      OR\n"
	"        tpcc_load [warehouse] [part] [min_wh] [max_wh]\n\n"
	...
	if(particle_flg==1){
	    part_no = atoi(argv[2]);
	    min_ware = atoi(argv[3]);
	    max_ware = atoi(argv[4]);
	}else{
	    min_ware = 1;
	    max_ware = count_ware;
	}
...
#else
                PrintBind(param, 9, warehouse_file);
#endif
...
#else
                PrintBind(param, 11, district_file);
#endif
...
#else
                PrintBind(param, 18, customer_file);
#endif
...
#else
                PrintBind(param, 8, history_file);
#endif
...
#else
                    PrintBind(param, 3, new_orders_file);
#endif
...
#else
                            PrintBind(param, 9, order_line_file);
#endif
			} else {
			    /*EXEC SQL INSERT INTO
				    order_line
...
#else
                            PrintBind(param, 10, order_line_file);
#endif
			}
