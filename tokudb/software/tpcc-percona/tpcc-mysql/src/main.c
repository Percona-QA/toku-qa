...
int main( int argc, char *argv[] )
{
  ...
  /* Parse args */
    ...
    while ( (c = getopt(argc, argv, "h:P:S:d:u:p:w:c:r:l:i:f:t:n:")) != -1) {
        switch (c) {
        case 'h':
        ...
        case 'n':
			...
			num_neword_per_10_sec = atoi(optarg);
            break;
        case '?':
    	    printf("Usage: tpcc_start -h server_host -P port -S socket -d database_name -u mysql_user -p mysql_password -w warehouses -c connections -r warmup_time -l running_time -i report_interval -f report_file -t trx_file -n new_order_per_10s\n");
            exit(0);
        default:
...
*/
  ...
  if(strcmp(connect_string,"localhost")==0){
    is_local = 1;
  }else{
    is_local = 0;
  }
