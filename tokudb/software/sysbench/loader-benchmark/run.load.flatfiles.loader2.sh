...
while [ ${TABLE_NUM} -le ${NUM_TABLES} ]; do
    ...
    if [ ${EVEN_TEST} -eq 1 ]; then
        ...
        $DB_DIR/bin/mysql --user=${MYSQL_USER} --socket=$MYSQL_SOCKET ${MYSQL_DATABASE}${DATABASE_NUM} -e "load data infile '$FULL_FILE_PATH' into table sbtest${TABLE_NUM} fields terminated by ',' enclosed by '\"';"
    fi
