...
void *batch_runner_proc(void *arg)
{
  ...
  while(batch_status != BATCH_STATUS_STOP)
  {
    ...
    {
      total = merge_timers(&total,&(sb_globals.op_timers[i]));
      get_reset_interval_max_time(&(sb_globals.op_timers[i]));
    }

    cur = diff_timers(&total, &prev);
    prev = total;
    ...
    log_text(LOG_NOTICE, "min: %.2f  avg: %.4f  max: %.4f  percentile: %.4f  throughput: %.2f  requests: %d  interval_secs: %d sum_secs: %.0f  threads: %d",
             NS2SEC(get_min_time(&cur))*1000.0, NS2SEC(get_avg_time(&cur))*1000.0,
             NS2SEC(get_reset_interval_max_time(&cur))*1000.0, NS2SEC(optime),
             cur.events / (double) batch_delay, (int) cur.events, (int) batch_delay, NS2SEC(cur.sum_time), (int) sb_globals.num_threads);
  }
