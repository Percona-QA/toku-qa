...
{
  {"oltp-test-mode", "test type to use {simple,complex,nontrx,sp,simplejoin,incinsert,incupdate,incupdate2}",
   SB_ARG_TYPE_STRING, "complex"},
 ...
 {"oltp-point-select-mysql-handler-open-read-close", "Use MySQL HANDLER open-read-close for point select",
   SB_ARG_TYPE_FLAG, "off"},
  {"oltp-simple-update", "do non-index update in simple test mode", SB_ARG_TYPE_FLAG, "off"},
...
{
  TEST_MODE_SIMPLE,
  TEST_MODE_SIMPLE_JOIN,
  TEST_MODE_COMPLEX,
  TEST_MODE_NONTRX,
  TEST_MODE_SP,
  TEST_MODE_INC_INSERT,
  TEST_MODE_INC_UPDATE,
  TEST_MODE_INC_UPDATE2
} oltp_mode_t;
...
typedef struct
{
  ...
  unsigned int     point_select_mysql_handler_orc;
  unsigned int     point_select_all_cols;
  unsigned int     secondary;
  unsigned int     simple_update;
} oltp_args_t;
...
typedef struct
{
  ...
  db_stmt_t *delete;
  db_stmt_t *insert;
  db_stmt_t *inc_insert;
  db_stmt_t *inc_update;
  db_stmt_t *inc_update2;
} oltp_stmt_set_t;
...
typedef struct
{
  ...
  sb_sql_query_insert_t inc_insert;
  sb_sql_query_update_t inc_update;
  sb_sql_query_update_t inc_update2;
...
static sb_request_t get_request_simple_join(int);
...
static sb_request_t get_request_inc(int tid, int do_update);
...
static db_stmt_t *get_sql_statement_inc(sb_sql_query_t *, int);
...
static int prepare_stmt_set_inc(oltp_stmt_set_t *, oltp_bind_set_t *, db_conn_t *);
...
int oltp_cmd_prepare(void)
{
  ...
  query_len = MAX_QUERY_LEN + nrows *
             (20 + /* id */
              20 + /* k */
             120 + /* c char(120) */
              60 + /* pad char(60) */
              16 + /* space + comma + 2 quotes per field */
               1); /* nul terminator */
...
void oltp_print_mode(void)
{
  ...
  switch (args.test_mode) {
    case TEST_MODE_SIMPLE:
      ...
      break;
    case TEST_MODE_SIMPLE_JOIN:
      log_text(LOG_NOTICE, "Running simple join OLTP test");
      break;
    ...
    case TEST_MODE_NONTRX:
      ...
      break;
    case TEST_MODE_INC_INSERT:
      log_text(LOG_NOTICE, "Running increment-insert test");
      break;
    case TEST_MODE_INC_UPDATE:
      log_text(LOG_NOTICE, "Running increment-update test");
      break;
    case TEST_MODE_INC_UPDATE2:
      log_text(LOG_NOTICE, "Running increment-update2 test");
      break;
    case TEST_MODE_SP:
...
sb_request_t oltp_get_request(int tid)
{
  ...
  switch (args.test_mode) {
    ...
    case TEST_MODE_SIMPLE_JOIN:
      return get_request_simple_join(tid);
    case TEST_MODE_COMPLEX:
    ...
    case TEST_MODE_INC_INSERT:
      return get_request_inc(tid, 0);
    case TEST_MODE_INC_UPDATE:
      return get_request_inc(tid, 1);
    case TEST_MODE_INC_UPDATE2:
      return get_request_inc(tid, 2);
    default:
...
sb_request_t get_request_inc(int tid, int do_update)
{
  sb_request_t        sb_req;
  sb_sql_request_t    *sql_req = &sb_req.u.sql_request;
  sb_sql_query_t      *query;
  ...
  if (do_update)
  {
    ...
    query->think_time = get_think_time();
    if (do_update == 1)
      query->type = SB_SQL_QUERY_INC_UPDATE;
    else
      query->type = SB_SQL_QUERY_INC_UPDATE2;
    query->u.update_query.id = GET_RANDOM_ID();
    SB_LIST_ADD_TAIL(&query->listitem, sql_req->queries);
  ...
  else
  {
    ...
    query->num_times = 1;
    query->think_time = get_think_time();
    query->type = SB_SQL_QUERY_INC_INSERT;
    query->u.insert_query.id = GET_RANDOM_ID();
    SB_LIST_ADD_TAIL(&query->listitem, sql_req->queries);
  }
...
sb_request_t get_request_simple_join(int tid)
{
  sb_request_t        sb_req;
  sb_sql_request_t    *sql_req = &sb_req.u.sql_request;
  sb_sql_query_t *query;
  ...
  if (range < 1)
    range = 1;     
  query->u.join_query.from = range;
  query->u.join_query.to = range + args.range_size - 1;
  ...
  do  /* deadlock handling */
  {
    ...
    SB_LIST_FOR_EACH(pos, sql_req->queries)
    {
      ...
      switch(query->type) {
        ...
        case SB_SQL_QUERY_INC_INSERT:
        case SB_SQL_QUERY_INC_UPDATE:
        case SB_SQL_QUERY_INC_UPDATE2:
...
int parse_arguments(void)
{
  ...
  else if (!strcmp(s, "simplejoin"))
    args.test_mode = TEST_MODE_SIMPLE_JOIN;
  ...
  else if (!strcmp(s, "incinsert"))
    args.test_mode = TEST_MODE_INC_INSERT;
  else if (!strcmp(s, "incupdate"))
    args.test_mode = TEST_MODE_INC_UPDATE;
  else if (!strcmp(s, "incupdate2"))
    args.test_mode = TEST_MODE_INC_UPDATE2;
  else
  {
  ...
  args.point_select_mysql_handler_orc =
      sb_get_value_flag("oltp-point-select-mysql-handler-open-read-close");
  args.simple_update = sb_get_value_flag("oltp-simple-update");
  ...
  {
    log_text(LOG_FATAL,
             "Must set --oltp-auto-inc for inc-update and inc-insert test modes");
    return 1;
  }
...
int prepare_stmt_set(oltp_stmt_set_t *set, oltp_bind_set_t *bufs, db_conn_t *conn)
{
  ...
  else if (args.test_mode == TEST_MODE_INC_UPDATE ||
           args.test_mode == TEST_MODE_INC_UPDATE2 ||
           args.test_mode == TEST_MODE_INC_INSERT)
    return prepare_stmt_set_inc(set, bufs, conn);
...
void close_stmt_set(oltp_stmt_set_t *set)
{
  ...
  db_close(set->delete);
  db_close(set->insert);
  db_close(set->inc_insert);
  db_close(set->inc_update);
  db_close(set->inc_update2);
...
db_stmt_t *get_sql_statement(sb_sql_query_t *query, int thread_id)
{
  if (args.test_mode == TEST_MODE_NONTRX)
    ...
    return get_sql_statement_trx(query, thread_id);
           ...
           args.test_mode == TEST_MODE_INC_UPDATE2 ||
           args.test_mode == TEST_MODE_INC_INSERT)
    return get_sql_statement_inc(query, thread_id);
...
int prepare_stmt_set_trx(oltp_stmt_set_t *set, oltp_bind_set_t *bufs, db_conn_t *conn)
{
  ...
  else if  (args.point_select_mysql_handler_orc)
  {
    snprintf(query, MAX_QUERY_LEN, "HANDLER %s OPEN_READ_CLOSE `%s` = (?)",
             args.table_name,
             args.secondary ? "xid" : "PRIMARY");
  }
  else
...
/* Prepare the join statement */
  snprintf(query, MAX_QUERY_LEN,
           "SELECT t1.c, t2.c from %s t1, %s t2 where t1.id between ? and ? and t2.id = %d - t1.id",
           args.table_name, args.table_name, args.table_size);
  set->join = db_prepare(conn, query);
  if (set->join == NULL)
    return 1;
  binds[0].type = DB_TYPE_INT;
  ...
  binds[1].is_null = 0;
  binds[1].data_len = 0;
  if (db_bind_param(set->join, binds, 2))
    return 1;
...
/* Prepare a set of statements for increment test */


int prepare_stmt_set_inc(oltp_stmt_set_t *set, oltp_bind_set_t *bufs, db_conn_t *conn)
{
  db_bind_t binds[11];
  char      query[MAX_QUERY_LEN];

  /* Prepare the update_index statement */
  snprintf(query, MAX_QUERY_LEN, "UPDATE %s set c=c+1 where id=?",
           args.table_name);
  set->inc_update = db_prepare(conn, query);
  if (set->inc_update == NULL)
    return 1;
  binds[0].type = DB_TYPE_INT;
  binds[0].buffer = &bufs->inc_update.id;
  binds[0].is_null = 0;
  binds[0].data_len = 0;
  if (db_bind_param(set->inc_update, binds, 1))
    return 1;

  /* Prepare the other update_index statement */
  snprintf(query, MAX_QUERY_LEN, "UPDATE %s set c= (case when c > 1000000000 then 0 else c end) + 1 where id=?",
           args.table_name);
  set->inc_update2 = db_prepare(conn, query);
  if (set->inc_update2 == NULL)
    return 1;
  binds[0].type = DB_TYPE_INT;
  binds[0].buffer = &bufs->inc_update2.id;
  binds[0].is_null = 0;
  binds[0].data_len = 0;
  if (db_bind_param(set->inc_update2, binds, 1))
    return 1;

  /* Prepare the insert statement */
  snprintf(query, MAX_QUERY_LEN, "INSERT INTO %s values(?,0,'0',"
           "'aaaaaaaaaaffffffffffrrrrrrrrrreeeeeeeeeeyyyyyyyyyy')"
           " ON DUPLICATE KEY UPDATE c = c + 1",
           args.table_name);
  set->inc_insert = db_prepare(conn, query);
  if (set->inc_insert == NULL)
    return 1;
  binds[0].type = DB_TYPE_INT;
  binds[0].buffer = &bufs->inc_insert.id;
  binds[0].is_null = 0;
  binds[0].data_len = 0;
  if (db_bind_param(set->inc_insert, binds, 1))
    return 1;
...
db_stmt_t *get_sql_statement_trx(sb_sql_query_t *query, int thread_id)
{
  ...
  switch (query->type) {
    ...
    case SB_SQL_QUERY_JOIN:
      stmt = statements[thread_id].join;
      buf->range.from = query->u.join_query.from;
      buf->range.to = query->u.join_query.to;
      break;
...
/* Generate SQL statement from query for increment test */


db_stmt_t *get_sql_statement_inc(sb_sql_query_t *query, int thread_id)
{
  db_stmt_t       *stmt = NULL;
  db_bind_t       binds[2];
  oltp_bind_set_t *buf = bind_bufs + thread_id;
  ...
  switch (query->type) {
    ...
    case SB_SQL_QUERY_INC_UPDATE:
      stmt = statements[thread_id].inc_update;
      buf->inc_update.id = query->u.update_query.id;
      break;

    case SB_SQL_QUERY_INC_UPDATE2:
      stmt = statements[thread_id].inc_update2;
      buf->inc_update2.id = query->u.update_query.id;
      break;

    case SB_SQL_QUERY_INC_INSERT:
      stmt = statements[thread_id].inc_insert;
      buf->inc_insert.id = query->u.insert_query.id;
      break;
