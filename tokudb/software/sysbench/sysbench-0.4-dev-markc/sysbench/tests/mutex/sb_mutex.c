...
static sb_arg_t mutex_args[] =
{
  ...
  {"mutex-type",
   "type of mutex: mutex, mutex-adaptive, rwlock-read, rwlock-write",
   SB_ARG_TYPE_STRING, "mutex"},
  {NULL, NULL, SB_ARG_TYPE_NULL, NULL}
}; 

typedef enum
{
   SB_MUTEX, SB_MUTEX_ADAPTIVE, SB_RWLOCK_READ, SB_RWLOCK_WRITE
} sb_mutex_type;
...
static sb_mutex_type mutex_type;
...
int mutex_execute_request(sb_request_t *sb_req, int thread_id)
{
  ...
  do
  {
    ...
    else if (mutex_type == SB_RWLOCK_READ)
      pthread_rwlock_rdlock(&thread_locks[current_lock].rwlock);
    else
      pthread_rwlock_wrlock(&thread_locks[current_lock].rwlock);
    ...
    if (mutex_type == SB_MUTEX || mutex_type == SB_MUTEX_ADAPTIVE)
      ...
      pthread_rwlock_unlock(&thread_locks[current_lock].rwlock);
