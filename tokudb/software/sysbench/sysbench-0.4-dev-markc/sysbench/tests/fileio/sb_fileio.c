...
{
  FILE_CHECKSUM_CRC32,
  FILE_CHECKSUM_INNODB_BYTE,
  FILE_CHECKSUM_INNODB_WORD
} file_checksum_t;
...
static file_checksum_t   file_checksum_function;
...
static sb_arg_t fileio_args[] = {
  ...
  {"file-checksum",
   "compute page checksum after each read and before each write, "
   "--validate determines whether the checksum result must be correct",
   SB_ARG_TYPE_FLAG, "off"},
  {"file-checksum-function", "checksum algorithm {crc32, innodb_byte, innodb_word}",
   SB_ARG_TYPE_STRING, "crc32"},
...
static unsigned long checksum_innodb(const char* buf, unsigned int len);
static unsigned long compute_checksum(const char* buf, unsigned int len);
...
int file_execute_request(sb_request_t *sb_req, int thread_id)
{
  ...
  switch (file_req->operation) {
    ...
    case FILE_OP_TYPE_WRITE:
      ...
      /* Store checksum and offset in a buffer when in checksum mode */
      if (file_checksum)
...
void file_print_mode(void)
{
  ...
  if (sb_globals.validate)
    log_text(LOG_INFO, "Using checksums with validation.");
  else if (file_checksum)
    log_text(LOG_INFO, "Using checksums without validation.");
  else
    log_text(LOG_INFO, "Not using checksums.");

  if (sb_globals.validate || file_checksum)
  {
    switch (file_checksum_function)
    {
      case FILE_CHECKSUM_CRC32:
        log_text(LOG_INFO, "Using crc32 for checksum.");
        break;
      case FILE_CHECKSUM_INNODB_BYTE:
        log_text(LOG_INFO, "Using byte-at-a-time InnoDB algorithm for checksum.");
        break;
      case FILE_CHECKSUM_INNODB_WORD:
        log_text(LOG_INFO, "Using word-at-a-time InnoDB algorithm for checksum.");
        break;
      default:
        log_text(LOG_FATAL, "Using unsupported checksum (%d).",
                 (int)file_checksum_function);
    }
  }
...
int parse_arguments(void)
{
  ...
  file_checksum = sb_get_value_flag("file-checksum");

  if (sb_globals.validate && !file_checksum)
  {
    log_text(LOG_FATAL,
             "Cannot use --validate unless also using --file-checksum.");
    return 1;
  }

  checksum_name = sb_get_value_string("file-checksum-function");
  if (checksum_name == NULL || !strcasecmp(checksum_name, "crc32"))
    file_checksum_function = FILE_CHECKSUM_CRC32;
  else if (!strcasecmp(checksum_name, "innodb_byte"))
    file_checksum_function = FILE_CHECKSUM_INNODB_BYTE;
  else if (!strcasecmp(checksum_name, "innodb_word"))
    file_checksum_function = FILE_CHECKSUM_INNODB_WORD;
  else
  {
    log_text(LOG_FATAL, "--file-checksum-function=%s is invalid", checksum_name);
    return 1;
  }
...
inline unsigned long checksum_innodb_pair(unsigned long n1, unsigned long n2)
{
  const unsigned long hash_mask = 1463735687;
  const unsigned long hash_mask2 = 1653893711;

  return(((((n1 ^ n2 ^ hash_mask2) << 8) + n1) ^ hash_mask) + n2);
} 

static unsigned long checksum_innodb_byte(const char* buf, unsigned int len)
{
  ...
  while (buf < end)
  {
    fold = checksum_innodb_pair(fold, (unsigned long)(*buf));
    buf++;
  }
...
static unsigned long checksum_innodb_word(const char* buf, unsigned int len)
{
  unsigned int len_word = len / sizeof(unsigned long);
  const unsigned long* buf_word = (unsigned long*) buf;
  const unsigned long* end = buf_word + len_word;
  unsigned long fold = 0;

  while (buf_word < end)
  {
    fold = checksum_innodb_pair(fold, *buf_word);
    buf_word++;
  }
...
static unsigned long compute_checksum(const char* buf, unsigned int len)
{
  switch (file_checksum_function)
  {
    ...
    case FILE_CHECKSUM_INNODB_BYTE:
      return checksum_innodb_byte(buf, len);
    case FILE_CHECKSUM_INNODB_WORD:
     return checksum_innodb_word(buf, len);
    default:
     log_text(LOG_FATAL,  "unsupported file_checksum_function (%d)",
              (int) file_checksum_function);
     return 0;
  }
}
...
void file_fill_buffer(char *buf, unsigned int len, size_t offset)
{
  ...
  /* Store the checksum */
  *(int *)(buf + i) = (int)compute_checksum(buf, len -
...
int file_validate_buffer(char  *buf, unsigned int len, size_t offset)
{
  ...
  checksum = (unsigned int)compute_checksum(buf, cs_offset);
