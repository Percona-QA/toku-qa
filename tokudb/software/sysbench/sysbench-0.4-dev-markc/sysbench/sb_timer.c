...
void sb_timer_stop(sb_timer_t *t)
{
  ...
  if (t->my_time > t->interval_max_time)
    t->interval_max_time = t->my_time;
...
unsigned long long  get_reset_interval_max_time(sb_timer_t *t)
{
  unsigned long long r = t->interval_max_time;
  t->interval_max_time = 0;
  return r;
}
...
/* Subtract t2 from t1 */
sb_timer_t diff_timers(sb_timer_t *t1, sb_timer_t *t2)
{
  sb_timer_t t;
  ...
  if (t1->interval_max_time > t2->interval_max_time)
    t.interval_max_time = t1->interval_max_time;
  else 
    t.interval_max_time = t2->interval_max_time;
