...
double db_get_tps(sb_stat_t type)
{
  double        seconds;
  unsigned int  i;
  unsigned long read_ops;
...
//  log_text(LOG_NOTICE, "db_get_tps(1)");
  ...
  if (type == SB_STAT_INTERMEDIATE)
  {
    //seconds = NS2SEC(sb_timer_split_tmc(&sb_globals.exec_timer));
    //seconds = NS2SEC(sb_timer_value(&sb_globals.exec_timer));
    //tps = ((double) transactions - (double) last_transactions) / seconds;
    tps = transactions;
    //tps = seconds;
  }
  else
  {
    //seconds = NS2SEC(sb_timer_split_tmc(&sb_globals.cumulative_timer1));
    tps = transactions;
  }
  return tps;
}
