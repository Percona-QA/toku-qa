...
static double rate_last_transactions = 0.0;
...
//  }
  ...
  do
  {
//    log_text(LOG_NOTICE, "runner_thread(1)");
...
//    if (sb_globals.tx_rate > 0)
//    {
      /* check how many transactions we've done this second */
      //tmc
      
//      log_text(LOG_NOTICE, "runner_thread(2)");

      // thread 0, restart interval every 10 seconds
      if (thread_id == 0)
      {
        current_seconds = NS2SEC(sb_timer_value(&sb_globals.cumulative_timer1));
        if (current_seconds > (rate_last_seconds + 10))
        { 
          rate_last_seconds = current_seconds; 
          rate_last_transactions = test->ops.get_tps(SB_STAT_INTERMEDIATE);
        } 
      }

      // check current rate and tarpit if necessary
      current_seconds = NS2SEC(sb_timer_value(&sb_globals.cumulative_timer1));
      current_transactions = test->ops.get_tps(SB_STAT_INTERMEDIATE);
      current_tps = (current_transactions - rate_last_transactions) / (current_seconds - rate_last_seconds);

      while (current_tps > sb_globals.tx_rate)
      {
        usleep(100000);
        current_seconds = NS2SEC(sb_timer_value(&sb_globals.cumulative_timer1));
        current_transactions = test->ops.get_tps(SB_STAT_INTERMEDIATE);
...
//        if (thread_id == 1)
//        {
//          log_text(LOG_NOTICE, "tarpitting thread %d : tps = %f : cur_secs = %f : last_secs = %f : cur_txn = %f : last_txn = %f",thread_id,current_tps,current_seconds,rate_last_seconds,current_transactions,rate_last_transactions);
//        }
      }
//      n = sprintf(buffer,"txn: %f",current_tps);
//      log_text(LOG_NOTICE, buffer);
      
//        usleep(500000);  

//      while (current_tps > 30)
//      {
        // sleep for 100 milliseconds
//        usleep(100000);  
//        current_tps = test->ops.get_tps(SB_STAT_INTERMEDIATE);
//        n = sprintf(buffer,"tps: %f",current_tps);
//        log_text(LOG_NOTICE, buffer);
//      }
      
//      log_text(LOG_NOTICE, "runner_thread(2) - unstuck");
//    }
