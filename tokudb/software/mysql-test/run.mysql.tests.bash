...
if [ -z $engine ] ; then
	...
	if [ $? = 0 ] ; then
	    for t in $tests ;  do
		    ...
		    if [ -d $t/t ] ; then teststorun="$teststorun,$t"; fi
	    done
	    popd
	fi
	...
	./mysql-test-run.pl --suite=$teststorun --big-test --mysqld=--loose-tokudb-debug=3072 --max-test-fail=0 --force --retry=1 --testcase-timeout=30 \
