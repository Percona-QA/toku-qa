...
FILE_DIR=upsert-mysqldump-${NUM_ROWS}
...
echo "`date` | loader 2 checking TokuDB sizing = `du -ch ${DB_DIR}/data/*.tokudb         | tail -n 1`" | tee -a $LOG_NAME
echo "`date` | loader 2 checking InnoDB sizing = `du -ch ${DB_DIR}/data/${MYSQL_DATABASE} | tail -n 1`" | tee -a $LOG_NAME
