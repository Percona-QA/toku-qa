...
if [ ${SKIP_DB_CREATE} == "N" ]; then
    ...
    $BIN --no-defaults --basedir=${DB_DIR} --datadir=${DB_DIR}/data  --port=${MYSQL_PORT} --pid-file=${DB_DIR}/data/pid.pid --core-file --socket=${MYSQL_SOCKET} --log-error=${DB_DIR}/data/error.log.out >  ${DB_DIR}/data/mysqld.out 2>&1 &
