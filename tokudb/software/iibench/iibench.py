...
DEFINE_integer('num_secondary_indexes', 3,'Number of secondary indexes.')
...
def create_table():
  ...
  cursor.execute('create table %s ( '
                 ...
                 'engine=%s %s' % (FLAGS.table_name, FLAGS.engine, extraDDL))

  if FLAGS.num_secondary_indexes >= 1:
    cursor.execute('create index marketsegment on %s (price, customerid)' % (FLAGS.table_name))
  if FLAGS.num_secondary_indexes >= 2:
    cursor.execute('create index registersegment on %s (cashregisterid, price, customerid)' % (FLAGS.table_name))
  if FLAGS.num_secondary_indexes >= 3:
    cursor.execute('create index pdc on %s (price, dateandtime, customerid)' % (FLAGS.table_name))
